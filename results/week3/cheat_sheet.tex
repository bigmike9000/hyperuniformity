...
\section{Slide 13: Metallic-mean convergence to $1/3$}

\Q{Why does the limit equal $1/3$?}
As $n\to\infty$, $\mu_n\to\infty$, so $L/S \to\infty$. But $\ell_L/\langle\ell\rangle \to 1$ --- the long tile becomes the mean spacing. Short tiles become rare (frequency $\sim 1/n$) and tiny. The chain looks like a lattice with rare defects, matching URL $a=1$ which has $\bar\Lambda = (1+1^2)/6 = 1/3$ (Klatt et al.\ 2020).

\Q{Is the convergence to $1/3$ proven?}
No, numerical observation. Computed $\bar\Lambda$ for $n=1$ through 20. Monotone from below, fits $1/3 - \bar\Lambda \sim 0.22/n^{1.43}$.

\Q{Is there a closed-form formula for $\bar\Lambda(\mu_n)$?}
Open question. Silver might be $1/4$. The limit is $1/3$. But we don't have a formula connecting them.

\Q{How is this related to URL?}
URL with parameter $a$ has $\bar\Lambda = (1+a^2)/6$. At $a=0$: perfect lattice ($\bar\Lambda = 1/6$). At $a=1$: maximally disordered ($\bar\Lambda = 1/3$). The metallic-mean chains interpolate between these as $n$ increases.
