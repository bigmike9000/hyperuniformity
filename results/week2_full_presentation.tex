...
Generated stealthy patterns with collective coordinate optimization.

\vspace{0.3em}
\textbf{Idea:} Force $S(k) = 0$ for all $|k| < K^*$, not just at $k = 0$.

\vspace{0.5em}
\textbf{Results} ($N = 10{,}000$):
\begin{center}
\begin{tabular}{cccc}
\toprule
$\chi$ & $\bar{\Lambda}$ & $\alpha$ & Iterations to converge \\
\midrule
0.1 & 0.166 & 2.0 & 328 \\
0.2 & 0.166 & 2.0 & 596 \\
0.3 & 0.166 & 2.0 & 694 \\
...
\vspace{0.3em}
These disordered patterns achieve optimal hyperuniformity. The optimization just moves points around until the structure factor vanishes in the exclusion region.
\end{frame}
\note{
Stealthy = $S(k) = 0$ for all $|k| < K^*$ (not just at zero).
