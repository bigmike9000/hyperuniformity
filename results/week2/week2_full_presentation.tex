...
\begin{itemize}
  ...
  \item New patterns: Class II (Period-Doubling) and Class III (0222)
  ...
  \item Updated ranking across all three classes
\end{itemize}
}
...
$\Rightarrow\alpha \approx 0.64$

\vspace{0.4em}
\textbf{Results:}
\begin{itemize}
    \item $\sigma^2 \sim R^{0.36}$
    \item Exponent $1-0.64=0.36$ \checkmark
...
\vspace{0.3em}
All three hyperuniformity classes now demonstrated.
\end{frame}
\note{
0222 has $|\lambda_2| > 1$, which gives $\alpha < 1$.

The figure shows Classes II and III side by side on log-log axes:
\begin{itemize}
  \item \textbf{Left (Class II):} Period-Doubling --- variance grows slowly, following $C\ln R + b$ (dashed). The trend is unmistakable but the growth is mild.
  \item \textbf{Right (Class III):} 0222 Chain --- variance climbs as a clear straight line on log-log, fitted by $R^{0.335}$, consistent with $R^{1-\alpha} = R^{0.36}$.
\end{itemize}

Class I (bounded variance) was covered in previous slides.
All three classes now have numerical evidence.
}
...
\textbf{Cloaking (if it comes up):} The structure factor of a perturbed lattice is
$S(k) = (1 - |\tilde{f}(k)|^2) + |\tilde{f}(k)|^2 S_L(k)$ (Klatt et al.\ Eq.\ 6).
The second term carries the Bragg peaks of the original lattice, attenuated by $|\tilde{f}|^2$.
For the URL, $\tilde{f}(k) = \mathrm{sinc}(ak/2)$, which vanishes at all non-zero reciprocal lattice
vectors exactly when $a \in \mathbb{Z}$. At $a = 1$ the displacement cell tiles the unit cell perfectly
--- all Bragg peaks vanish and the pattern is indistinguishable from a disordered system by any
two-point measurement. Our $a = 1.0$ data point is this \emph{cloaked} case.
$\bar{\Lambda}$ increases monotonically with $a$ (more fluctuation), but the $\tau$ order metric
oscillates --- finite (no long-range order detectable) at $a=1$, diverging (Bragg peaks back) at $a=1.2$.
}
