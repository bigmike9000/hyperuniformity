...
One subtlety worth knowing: at $a = 1$ the displacement is uniform over exactly one unit cell,
so the shifted cells tile the line perfectly with no gaps or overlaps. This causes the
characteristic function $\tilde{f}$ to vanish at every Bragg peak of the original lattice,
making the Bragg peaks disappear entirely from $S(k)$ --- the pattern becomes indistinguishable
from a disordered system by any two-point measurement. Klatt et al.\ call this ``cloaking.''
Our $a = 1.0$ data point is the cloaked case. Interestingly, $\bar{\Lambda}$ grows
monotonically with $a$ (stronger perturbations always increase density fluctuations), but the
underlying long-range order reappears at $a = 1.2$ when cells start to overlap.
}
