...
\centering
\includegraphics[width=\linewidth]{figures/figA_variance_by_class.png}
\end{column}
\end{columns}
\end{frame}
\note{
...
The figure shows all three classes' variance on log-log axes:
\begin{itemize}
  \item \textbf{Left (Class I):} $\sigma^2(R)$ saturates --- Fibonacci, Silver, Bronze, URL all flatten out.
  \item \textbf{Centre (Class II):} Period-Doubling grows like $C\ln R + b$ (dashed fit).
  \item \textbf{Right (Class III):} 0222 chain grows like $R^{0.335}$ (dashed fit) --- consistent with $R^{1-\alpha} = R^{0.36}$.
\end{itemize}

All three classes now covered with numerical evidence.
}
