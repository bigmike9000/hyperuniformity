"""
Investigation 4: Generalized Scaling Metrics for Class II and III
...
# ============================================================
print("=" * 65)
print("  Investigation 4: Generalized Scaling Metrics")
print("=" * 65)
...
print(f"    C_II (from plateau of sigma^2/ln R) = {C_II:.5f} ± {C_II_err:.5f}")
print(f"    C_II (from curve fit)               = {C_pd_fit:.5f}")
...
try:
    ...
    A_0222_fit = beta_0222_fit = alpha_0222_num = np.nan
    print(f"    Curve fit failed: {e}")

# Generalized metric using theoretical alpha
A_III, A_III_err, norm_0222 = compute_generalized_metric(
    R_0222, var_0222, 'III', alpha=alpha_0222)
print(f"    A_III (theory alpha={alpha_0222:.4f}) = {A_III:.5f} ± {A_III_err:.5f}")
...
ax.axhline(C_II, color='r', ls='--', lw=2, label=rf'$C_{{II}} = {C_II:.4f}$')
ax.fill_between([R_pd[use_pd][0], R_pd[use_pd][-1]],
                C_II - C_II_err, C_II + C_II_err, alpha=0.15, color='r')
ax.axhline(C_pd_fit, color='k', ls=':', lw=1.5, alpha=0.7,
           label=rf'Fit $C = {C_pd_fit:.4f}$')
ax.set_xlabel(r'$R$', fontsize=12)
...
ax.grid(True, ls=':', alpha=0.4)
ax.text(0.05, 0.95,
        rf'$C_{{II}} = {C_II:.4f} \pm {C_II_err:.4f}$',
        transform=ax.transAxes, va='top', fontsize=10,
        bbox=dict(boxstyle='round', fc='lightyellow', alpha=0.9))
...
ax.axhline(A_III, color='r', ls='--', lw=2, label=rf'$A_{{III}} = {A_III:.4f}$')
ax.fill_between([R_0222[use_0222][0], R_0222[use_0222][-1]],
                A_III - A_III_err, A_III + A_III_err, alpha=0.15, color='r')
if not np.isnan(A_0222_fit):
    ax.axhline(A_0222_fit, color='k', ls=':', lw=1.5, alpha=0.7,
               label=rf'Fit $A = {A_0222_fit:.4f}$')
ax.set_xlabel(r'$R$', fontsize=12)
...
ax.grid(True, ls=':', alpha=0.4)
ax.text(0.05, 0.95,
        rf'$\alpha_{{th}} = {alpha_0222:.4f}$, $\alpha_{{num}} = {alpha_0222_num:.4f}$'
        '\n' rf'$A_{{III}} = {A_III:.4f} \pm {A_III_err:.4f}$',
        transform=ax.transAxes, va='top', fontsize=10,
        bbox=dict(boxstyle='round', fc='lightyellow', alpha=0.9))
...
print(f"    C_II  = {C_II:.5f} ± {C_II_err:.5f}  [lim sigma^2(R)/ln R]")
print(f"    C_II  = {C_pd_fit:.5f}  [from curve fit]")
print()
print(f"  Class III — 0222 Chain (alpha={alpha_0222:.4f}):")
print(f"    A_III = {A_III:.5f} ± {A_III_err:.5f}  [lim sigma^2(R)/R^{{1-alpha}}]")
print(f"    A_III = {A_0222_fit:.5f}  [from curve fit]")
